...
Создает managed MySQL кластер с репликацией

Настраивает хосты в разных зонах для отказоустойчивости

Создает базу данных и пользователя для WordPress

Назначает внешние IP для доступа к БД
...
Создает балансировщик нагрузки для распределения трафика между инстансами

Автоматически добавляет все инстансы WordPress в целевую группу

Настраивает health check для мониторинга доступности инстансов

Открывает внешний IP на порту 80 для HTTP трафика
...
Отображает публичные IP адреса инстансов для SSH доступа

Показывает внешний IP балансировщика для доступа к WordPress

Выводит информацию о MySQL хостах для настройки подключения

Предоставляет данные для подключения к базе данных
...
├─────────────────────────────────────────────────────────────┤
│  ┌─────────────┐    ┌─────────────┐    ┌─────────────┐    │
│  │   Зона A    │    │   Зона B    │    │   Зона D    │    │
│  │  (wp-app-1) │    │  (wp-app-2) │    │  (MySQL)    │    │
│  │  10.2.0.0/16│    │  10.3.0.0/16│    │  10.4.0.0/16│    │
│  └──────┬──────┘    └──────┬──────┘    └──────┬──────┘    │
│         │                  │                   │           │
│  ┌──────┴──────────────────┴───────────────────┴──────┐    │
│  │              Виртуальная сеть wp-network           │    │
│  └──────────────────────────┬─────────────────────────┘    │
│                             │                              │
│                    ┌────────┴────────┐                     │
│                    │ Балансировщик   │                     │
